...
def get_multiplier(text):
    ...
    # 1. 분할 인쇄 (나누기) - '4페이지', '4up', '4쪽' 등에서 숫자 추출
    div_val = 1.0
...
def get_category(filename):
    ...
    # 2. TOC 판별 (Protocol 단어 내 toc 제외)
    # 단어 경계(\b)를 사용하여 독립적인 toc만 찾거나 특정 단어 매칭
    is_toc = False
    if any(k in fn for k in ['tableofcontents', '목차']): 
        is_toc = True
    elif re.search(r'\btoc\b|_toc|toc_', fn): # toc가 독립된 단어일 때만
        is_toc = True
...
# --- [메인 시스템] ---
st.set_page_config(page_title="무결점 에이전트 V7.5", layout="wide")
st.title("📂 사내 견적 자동화 에이전트 팀 (V7.5)")
...
if uploaded_zip:
    ...
    try:
        ...
        # 화면 출력
        st.subheader("📊 1. 최상위 폴더별 견적 요약")
        st.dataframe(pd.DataFrame.from_dict(summary, orient='index'))
        
        st.subheader("🔍 2. 상세 계산 근거 (오류 검증용)")
        ...
        st.download_button("📂 정밀 수정 견적서 다운로드", data=output.getvalue(), file_name="최종_견적_V7.5.xlsx")
