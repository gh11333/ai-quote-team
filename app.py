...
# PPTX 처리를 위해 추가 (설치 필요: pip install python-pptx)
try:
...
def extract_quantities(text):
    ...
    mul_val = 1
    # Golden Rule 2: 숫자 뒤에 반드시 '부' 또는 '장'이 붙어야 배수로 인정 [cite: 1, 2]
...
def get_accessory_count(text, item_name, default_mul=1):
    ...
    # Golden Rule 3: 절대 수량 전수 합산 (숫자 지시 우선) 
    ...
    # Golden Rule 3: 개별 모드 (각, 각각, 하나씩) -> 부수만큼 
...
def is_skip_printing(text):
    t = text.lower()
    # Golden Rule 5: 'USB', 'CD' 단어 경계 판독 및 'CDMS' 예외 처리 [cite: 4]
...
st.title("📂 2026 사내 견적 자동화 (V24.0 - 형제 상속 및 PPTX 대응 버전)")
...
if uploaded_zip:
    ...
    try:
        with zipfile.ZipFile(uploaded_zip, 'r') as z:
            ...
            # 1. 사전 지휘소: 지시서 및 형제 폴더 정보 수집
            folder_notes = {}
            sibling_notes = {} # Golden Rule 1: 형제 상속용 
            ...
            # 2. 메인 분석 엔진
            valid_files = [p for p in raw_paths if not p.endswith('/') and not p.lower().endswith(('.doc', '.docx', '.txt', '.msg'))] [cite: 8]
            ...
            for f in valid_files:
                ...
                # 계층적 지시 수집 (부모 + 형제 상속) [cite: 10, 11]
                inherited_instr = ""
                skip_reason = ""
                curr = foldername
                while True:
                    # 상위 폴더명 + 지시서 + 형제 폴더명 결합 [cite: 1, 6]
                ...
                m_vinyl = get_accessory_count(combined_low, '비닐', final_mul) [cite: 14]
